...
// ===========================
// NAV CLICK => CHARGE ANIM => NAVIGATE
// ===========================
document.addEventListener("click", (e) => {
  const btn = e.target.closest(".marqueeBtn");
  if (!btn) return;

  // If Home is disabled on the home page, do nothing
  ...
  const href = btn.getAttribute("href"); // "404.html" or "#home"

  playChargeAnimation(() => {
    if (!href) return;

    // allow hashes if you keep any
    if (href.startsWith("#")) {
      location.hash = href;
      return;
    }

    // otherwise navigate to real page
    window.location.href = href;
  });
});


// Firefox-safe nav hover detection
document.addEventListener("DOMContentLoaded", () => {
